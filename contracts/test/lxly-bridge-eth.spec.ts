...
describe("LXLY ETH Bridge Tests", function () {
  ...
  async function deployBridge() {
    ...
    const RollupBridge = await ethers.getContractFactory("RollupETHBridge");
  ...
  describe("Deposit", () => {
    ...
    it("Should  bridge an erc20 token successfully", async function () {
      ...
      const bridgeTxn = rollupBridge.bridge(
        amount,
        usdcToken,
        "0xA8CE8aee21bC2A48a5EF670afCc9274C7bbbC035", //usdc Address on destination
        otherAccount.address,
        true
      );
      const rollUpAddr = await rollupBridge.getAddress();
    ...
    it("Should swap and bridge a token successfully", async function () {
      const { rollupBridge, otherAccount, polygonZkEVMBridge, zeroAddress } =
        await loadFixture(deployBridge);
      const outputToken = "0xA8CE8aee21bC2A48a5EF670afCc9274C7bbbC035";
      const recipient = otherAccount.address;
      const bridgeContract = new Contract(
        polygonZkEVMBridge,
        bridge.abi,
    ...
    it("Should swap and bridge an erc20 token successfully", async function () {
      ...
      await usdcContract.approve(await rollupBridge.getAddress(), amount);
      const recipient = otherAccount.address;
      const outputToken = "0x4F9A0e7FD2Bf6067db6994CF12E4495Df938E6e9"; //bridge wrapped ether
      const bridgeTxn = rollupBridge.bridge(
        amount,
        usdcToken,
      ...
      //   await expect(bridgeTxn).to.not.be.reverted;
      await expect(bridgeTxn).to.emit(bridgeContract, "BridgeEvent").withArgs(
        0,
        0,
        zeroAddress, //get raw ether
        1,
        recipient,
        anyValue,
        anyValue,
